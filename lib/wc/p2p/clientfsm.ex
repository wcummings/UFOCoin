...
defmodule WC.P2P.ClientFSM do
  ...
  def child_spec([n]) do
    %{
      id: String.to_atom(Atom.to_string(__MODULE__) <> Integer.to_string(n)), # Unique id's
      restart: :permanent,
      shutdown: 5000,
  ...
  def handle_event(:timeout, _, :starting_handshake, data = %{socket: socket}) do  
    send_packet(socket, %P2PPacket{proc: :version, extra_data: WC.version})
